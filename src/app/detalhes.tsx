...
import { adicionarFilme, filmeJaExiste } from "../services/firestore";
...
export default function DetalhesScreen() {
  ...
  async function verificarFilme() {
    try {
      ...
      console.log("Erro ao verificar filme:", error);
    } finally {
  ...
  async function salvarFilme() {
    ...
    try {
      ...
      Alert.alert("Pronto!", `${filme.titulo} foi adicionado à sua lista.`);
    } catch (error) {
      console.log(error);
  ...
  return (
    <View style={styles.container}>
      ...
      <TouchableOpacity
        style={[styles.botao, adicionado && styles.botaoAdicionado]}
        onPress={salvarFilme}
        disabled={adicionado || salvando || verificando}
      >
        {salvando || verificando ? (
          <ActivityIndicator color="#fff" />
        ) : (
          <Text style={styles.textoBotao}>
            {adicionado ? "Filme adicionado" : "Adicionar à minha lista"}
          </Text>
        )}
      </TouchableOpacity>
      ...
      >
        <Text style={styles.textoBotaoSecundario}>Ver minha lista</Text>
      </TouchableOpacity>
    </View>
  );
}
